...
const CreatePRDialogImpl = NiceModal.create<CreatePRDialogProps>(
  ({ attempt, task, repoId, targetBranch }) => {
    ...
    const { environment, config, updateAndSaveConfig } = useUserSystem();
    ...
    const [openInBrowser, setOpenInBrowser] = useState(
      config?.open_pr_in_browser ?? true
    );
    ...
    // persiste el estado del toggle de auto-generación
    const handleAutoGenerateChange = useCallback(
      (checked: boolean | 'indeterminate') => {
        const value = checked === true;
        setAutoGenerateDescription(value);
        updateAndSaveConfig({ pr_auto_description_enabled: value });
      },
      [updateAndSaveConfig]
    );

    // persiste el estado del toggle de abrir en navegador
    const handleOpenInBrowserChange = useCallback(
      (checked: boolean | 'indeterminate') => {
        const value = checked === true;
        setOpenInBrowser(value);
        updateAndSaveConfig({ open_pr_in_browser: value });
      },
      [updateAndSaveConfig]
    );
    ...
    const handleConfirmCreatePR = useCallback(async () => {
      ...
      if (result.success) {
        ...
        setOpenInBrowser(config?.open_pr_in_browser ?? true);
        setCreatingPR(false);
    ...
    const handleCancelCreatePR = useCallback(() => {
      ...
      setOpenInBrowser(config?.open_pr_in_browser ?? true);
    }, [modal, config?.pr_auto_description_enabled, config?.open_pr_in_browser, error]);
          ...
          <DialogContent className="sm:max-w-[525px]">
            ...
            {!isLoaded ? (
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="pr-open-in-browser"
                    checked={openInBrowser}
                    onCheckedChange={handleOpenInBrowserChange}
                    className="h-5 w-5"
                  />
                  <Label
                    htmlFor="pr-open-in-browser"
                    className="cursor-pointer text-sm"
                  >
                    {t('createPrDialog.openInBrowserLabel')}
                  </Label>
                </div>
                {ghCliHelp?.variant && (
