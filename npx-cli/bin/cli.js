...
async function main() {
      ...
      .then((latest) => {
        if (latest && latest !== CLI_VERSION) {
          setTimeout(() => {
            ...
            console.log(`Run: npx @miermontoto/vkm@latest`);
          }, 2000);
        }
      })
      .catch(() => {});
  ...
  if (isMcpMode) {
    await extractAndRun("vkm-mcp", (bin) => {
    ...
    await extractAndRun("vkm-review", (bin) => {
    ...
    console.log(`Starting vkm v${CLI_VERSION}${modeLabel}...`);
    await extractAndRun("vkm", (bin) => {
      if (platform === "win32") {
